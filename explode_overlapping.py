...
Required Arguments:
    - fc (feature class in gdb): feature class to be divided into non-overlapping sets
    - tolerance (distance in fc coordinate system's linear unit): size of buffer around features to determine intersection
Optional Argument:
...
def ExplodeOverlapping(fc, idName = "ORIG_FID", tolerance=None, keep=True, overwrite = False):
    if arcpy.Exists(fc+'buf') and overwrite==False:
        print('Buffers already exist... skipping')
        fcbuf = fc + 'buf'
    elif tolerance > 0:
        print('Buffering...')
    ...
    if arcpy.Exists(fc+'intersect') and overwrite==False:
        print('Buffer intersections already exist... skipping')
    else:
        print('Intersecting buffers...')
        arcpy.Intersect_analysis(fcbuf, fc+'intersect')
