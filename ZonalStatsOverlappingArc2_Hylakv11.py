...
resgdb = os.path.join(resdir, 'mathis_hylak_v11_results.gdb')
seamask = os.path.join(os.path.split(rootdir)[0], 'HydroATLAS', 'results', 'glad.gdb', 'class99_19_agg9')
hlk10 = os.path.join(datdir, 'hydrolakes', 'HydroLAKES_polys_v10.gdb', 'HydroLAKES_polys_v10')
outlk = os.path.join(resgdb, 'mathis_hylak_v11_stats')

########
input_value_raster = os.path.join(resdir, 'eegtopo_slope')
# Vector dataset(zones)
input_zone_polygon = os.path.join(datdir, 'mathis_hylak_v11.gdb', 'mathis_hylak_v11')
#input_zone_polygon = os.path.join(resdir, 'slopetest.gdb', 'BClakes_slopetest')
# Output csv file
csv_folder = os.path.join(resdir, 'hylak_v11_statstabs')
stat = 'MEAN'
exist_buf = os.path.join(resdir, 'mathis_hylak_v11_buf100.shp')
#exist_buf = os.path.join(resdir, 'BClakes_slopetest_buf100.shp')
ID_field = "Mathis_id"
buf_size = ""
...
#Join output statistics table to polygons
arcpy.MakeFeatureLayer_management(input_zone_polygon, 'lklyr')
arcpy.AddJoin_management('lklyr', in_field= 'Mathis_id',
                         join_table= os.path.join(csv_folder, '{}.csv'.format(os.path.split(exist_buf)[1])),
                         join_field= 'ID')
arcpy.CopyFeatures_management('lklyr', outlk)
arcpy.Delete_management('lklyr')

#Delete ID field and rename mean slope field
arcpy.DeleteField_management(outlk, 'ID')
arcpy.AlterField_management(outlk, field='MEAN', new_field_name='Slope_100_v11', new_field_alias='Slope_100_v11')


############################################### CHECK LAKES THAT FALL IN SEA MASK ######################################
outstattab = os.path.join(resgdb, 'hlk10_seamask_stat')
ZonalStatisticsAsTable(in_zone_data=hlk10, zone_field='Hylak_id',
                       in_value_raster=seamask,
                       out_table= outstattab,
                       statistics_type='SUM')

arcpy.MakeFeatureLayer_management(hlk10, 'lklyr')
arcpy.AddJoin_management('lklyr', in_field='Hylak_id', join_table=outstattab, join_field='Hylak_id')
arcpy.SelectLayerByAttribute_management('lklyr', selection_type='NEW_SELECTION', where_clause='hlk10_seamask_stat.COUNT > 0')
arcpy.CopyFeatures_management('lklyr', os.path.join(resgdb, 'hylk10_seainters'))