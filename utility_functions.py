...
#Given a reference input extent and a list of datasets to compare, return a new list with only those datasets that intersect
...
def dlfile(url, outpath, outfile=None, ignore_downloadable=False,
           fieldnames=None, verifycertif=True,
    ...
    try:
        if is_downloadable(url) or ignore_downloadable==True:  # check that url is not just html
            ...
            if username != None and password != None:
                ...
                f = requests.get(url, allow_redirects=True, verify=verifycertif)
                print "downloading " + url
